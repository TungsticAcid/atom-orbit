...
两者共用同一套**三维着色**：**相位色**（实函数 → ±红/青，判据是**完整 ψ 的符号** `sign(R(r)·Y)`，故径向节点处的符号翻转也能看到；复函数 → 彩虹相位）或**支壳层色**（按 s/p/d/f 着色，不显示正负）。s 轨道（l=0）的角向函数是常数、正负恒定，相位色无信息，此时该选项自动置灰。三维、角度分布、截面三张视图的相位配色**统一为同一套**。
...
- **角度分布**：以 (θ, φ) 为自变量、自原点沿该方向引射线、长度为 `|Y|` 或 `|Y|²` 的**三维曲面**，可拖拽旋转。着色只由**实/复**决定——实函数按 `sign(Y)` 分 ±红/青，复函数按相位 `arg Y` 彩虹着色（与三维、截面**共用同一个 phaseColor**）；**与判据无关**：切 `|Y| ↔ |Y|²` 改的是曲面半径（轮廓），颜色不变。
