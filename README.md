...
> **阈值范围与推荐值**：阈值滑块用**对数刻度**覆盖 `0.02%–80%`（线性刻度下低端根本拖不到，而"要看到所有节面"的阈值常常在 1% 以下）。面板会按当前轨道给出**推荐阈值**（取该轨道最弱一层径向壳峰值的 40%）并提供一键采用——因为各层壳的峰值相差悬殊（3p `100/11.9`、4p `100/11.1/4.2`、5d `100/17.4/8.1`、4s `100/1.8/0.42/0.18`），任何一个固定默认值都必然在某些轨道上把外层壳整片切掉。

两者共用同一套**三维着色**：**相位色（正负瓣）**（实函数 → ±红/青，判据是**完整 ψ 的符号** `sign(R(r)·Y)`，故径向节点处的符号翻转也能看到；复函数 → 彩虹相位）或**支壳层色**（按 s/p/d/f 着色，不显示正负）。s 轨道（l=0）的角向函数是常数、正负恒定，相位色无信息，此时该选项自动置灰。三维、角度分布、截面三张视图的相位配色**统一为同一套**。
