...
**量子数实时联动**：滑块调节主量子数 `n`（1–6）、角量子数 `l`、磁量子数 `m`，`l` 上限随 `n`、`m` 范围随 `l` 自动收窄。

**三维轨道渲染** — 按概率密度 `|ψ|²` 实时生成，两种方式可切换：

| 方式 | 说明 |
|---|---|
| **粒子云** | 按 `|ψ|²` 重要性采样数万个点，统计上即真实电子分布 |
| **等值面** | 在网格上以*四面体行进*提取 `|ψ|² = 常数` 的实体表面，阈值可调 |
...
| Three.js **r147 全局构建** | r148 起非模块 `OrbitControls` 被移除；全局构建可在 `file://` 双击运行（ES Module 在 `file://` 下会被 CORS 拦截） |
